...
Summary:    TUIO touch plugin
Group:      Qt/Qt
Requires:   %{name}-qtcore = %{version}-%{release}

%description plugin-generic-tuiotouch
A plugin used for accepting touch events sent from a remote
source using the TUIO protocol
...
%package qtplatformheaders-devel
Summary:    Header-only module for encapsulating platform-specifics
Group:      Qt/Qt
Requires:   %{name}-qtplatformheaders-devel = %{version}-%{release}

%description qtplatformheaders-devel
This package contains inline classes that encapsulate platform-specific
information tied to runtime platform configuration
...
Summary:    The QtEglDeviceIntegration library
Group:      Qt/Qt
Requires(post):     /sbin/ldconfig
Requires(postun):   /sbin/ldconfig

%description qtegldeviceintegration
This package contains the QtEglDeviceIntegration library

%package qtegldeviceintegration-devel
Summary:    Development files for QtEglDeviceIntegration
Group:      Qt/Qt
Requires:   %{name}-qtegldeviceintegration = %{version}-%{release}

%description qtegldeviceintegration-devel
This package contains the files decessary to develop
applications that use QtEglDeviceIntegration
...
%post qtegldeviceintegration -p /sbin/ldconfig
%postun qtegldeviceintegration -p /sbin/ldconfig
